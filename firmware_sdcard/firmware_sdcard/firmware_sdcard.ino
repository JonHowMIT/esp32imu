...
enum State { IDLE, LOG, READ, WAIT, DONE };
State state;
...
static constexpr int SAMPLE_PER_SEC = 1000;
static constexpr int PERIOD_TICKS = TICKS_PER_SEC / SAMPLE_PER_SEC;

// how many samples to collect?
static constexpr int NUM_SAMPLES = 10 * SAMPLE_PER_SEC;
  ...
  for (;;) {
    ...
    if (state == State::IDLE) {
      ...
      if (seqnow >= NUM_SAMPLES) {
        ...
        state = State::WAIT;
        Serial.println("Hit R to send data");
      }
